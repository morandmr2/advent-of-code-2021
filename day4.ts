class Grid {
  private readonly xSets: Record<string, Set<number>>; // set per row
  private readonly ySets: Record<string, Set<number>>; // set per column
  ...
  constructor(values: number[][]) {
    ...
    for (let y = 0 ; y < values[0].length ; y++) {
      this.ySets[y] = new Set<number>();
      for(let x = 0 ; x < values.length ; x++) {
        this.xSets[x] = new Set<number>(values[x]);
        this.ySets[y].add(values[x][y])
      }
    }
  }

  checkNumber(bingoNumber: number) {
    const xResult = this.checkNumberFromSet(this.xSets, bingoNumber);
    if (xResult) {
      return this.getFinalValue();
    }

    const yResult = this.checkNumberFromSet(this.ySets, bingoNumber);
    if (yResult) {
      return this.getFinalValue();
    }
  ...
  private checkNumberFromSet(dimensionToCheck: Record<string, Set<number>>, bingoNumber: number): boolean {
    return Object.keys(dimensionToCheck).map(key => {
      if (dimensionToCheck[key].has(bingoNumber)) {
        dimensionToCheck[key].delete(bingoNumber);

        if (dimensionToCheck[key].size === 0) {
          return true;
        }
      }
      return false;
  ...
  getFinalValue() {
    let sum = 0;
    Object.keys(this.xSets).forEach(key => {
      sum += Array.from(this.xSets[key]).reduce((prev, next) => prev + next, 0);
    });
